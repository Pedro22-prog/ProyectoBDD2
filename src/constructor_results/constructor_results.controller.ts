...
export class ConstructorResultsController {
  ...
  async findOne(@Param('id') id: string) {
    return await this.constructorResultsService.findOne(id);
  }

  @Patch(':id')
  async update(
    @Param('id') id: string,
    @Body() updateConstructorResultDto: UpdateConstructorResultDto,
  ) {
  ...
  async remove(@Param('id') id: string) {
    await this.constructorResultsService.remove(id);
    return { message: `ConstructorResult with ID ${id} has been removed.` };
  }
}